...
//
// A ideia: bots de varredura pedem caminhos que um usuário real nunca pediria
// (/.env, /wp-login.php, /cgi-bin/…), muitas vezes com user-agents de ferramentas
// (sqlmap, nikto, curl). Pontuamos o path + o UA e, se um caminho SENSÍVEL
// respondeu 2xx/3xx (existe de verdade!), marcamos `hit` — potencial exposição.

const RULES = [
  // categoria, rótulo PT, score base, regex do path (case-insensitive)
...
// User-agents de ferramentas de ataque / scanners automatizados.
...
// UAs de bibliotecas/CLI: fortíssimo indício de automação (não é navegador).
...
// Bots legítimos de busca (marcamos como bot, mas benignos).
...
// Assinatura mínima de navegador real.
...
export function classifyRequest({ path = '/', ua = '', method = 'GET', status = 0 } = {}) {
  ...
  // 1) regra de caminho de maior severidade que casar
  for (const [cat, lbl, base, rx] of RULES) {
    if (rx.test(target)) {
      if (base > score) { score = base; category = cat.trim(); label = lbl; }
    }
  }

  // 2) sinais de user-agent
  ...
  if (scanner) {
    ...
    if (category === 'benign') { category = 'scanner'; label = 'Scanner de vulnerabilidade'; }
    else score += 8;
  }
  if (tool && !goodBot) {
    score += category === 'benign' ? 22 : 10;
    if (category === 'benign') { category = 'automation'; label = 'Cliente automatizado'; }
  }
  if (emptyUa) score += 12;
  ...
  // 4) "acertou o alvo?" — caminho sensível que respondeu com sucesso/redirect.
  ...
  // 5) classificação bot vs humano
  let isBot;
  if (goodBot || scanner || tool || emptyUa) isBot = true;
  else if (looksBrowser && category === 'benign') isBot = false;
  else if (looksBrowser) isBot = category !== 'benign' && score >= 55; // navegador pedindo /.env = provável bot mascarado
  else isBot = true;
...
// porta -> [serviço, score base, categoria]. Portas sensíveis pontuam alto.
const SERVICES = {
...
// Faixa de severidade legível (usada no mapa/UI e nas cores).
